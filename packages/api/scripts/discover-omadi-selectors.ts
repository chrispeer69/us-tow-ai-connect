/**
 * One-off, READ-ONLY discovery script for Omadi (omadi.com). Logs in,
 * navigates to the dispatch board / open-jobs list, opens the first row's
 * detail via plain URL navigation (never a click on a mutating control), and
 * dumps the DOM structure + a button inventory so we can identify stable
 * Accept / Decline selectors for the adapter.
 *
 ...
 *   OMADI_USERNAME / OMADI_PASSWORD
 *
 * Run:  OMADI_USERNAME=… OMADI_PASSWORD=… pnpm exec tsx scripts/discover-omadi-selectors.ts
 * Headed: HEADFUL=1 OMADI_USERNAME=… OMADI_PASSWORD=… pnpm exec tsx scripts/discover-omadi-selectors.ts
 *
 * Exit codes:
...
const LOGIN_URL = 'https://app.omadi.com/login';
const DISPATCH_URL = 'https://app.omadi.com/dispatch';
const NAV_TIMEOUT_MS = 60_000;
const DIAG_DIR = path.resolve(process.cwd(), '../../docs/diagnostics');

const watchdog = setTimeout(() => {
  // eslint-disable-next-line no-console
  console.error('[discover-omadi] WATCHDOG fired (4m) — exiting non-zero');
  process.exit(3);
}, 4 * 60_000);
watchdog.unref();
...
async function main() {
  const username = process.env.OMADI_USERNAME;
  const password = process.env.OMADI_PASSWORD;
  await ensureDiagDir();

  if (!username || !password) {
    // eslint-disable-next-line no-console
    console.log('[discover-omadi] no creds — OMADI_USERNAME / OMADI_PASSWORD not set; exiting 0');
    await fs.writeFile(
      path.join(DIAG_DIR, `omadi-selectors-${ts()}.json`),
  ...
  try {
    ...
    await page
      .screenshot({ path: path.join(DIAG_DIR, 'omadi-discovery.png'), fullPage: true })
      .catch(() => undefined);
    ...
    if (href) {
      const detailUrl = href.startsWith('http')
        ? href
        : `https://app.omadi.com${href.startsWith('/') ? '' : '/'}${href}`;
      try {
    ...
    // eslint-disable-next-line no-console
    console.error('[discover-omadi] failed:', (err as Error).message);
  } finally {
    await browser.close().catch(() => undefined);
  }

  const outfile = path.join(DIAG_DIR, `omadi-selectors-${ts()}.json`);
  await fs.writeFile(outfile, JSON.stringify(result, null, 2));
  // eslint-disable-next-line no-console
  console.log(`[discover-omadi] wrote ${outfile} (ok=${result.ok})`);
  clearTimeout(watchdog);
...
main().catch((e) => {
  // eslint-disable-next-line no-console
  console.error('[discover-omadi] uncaught:', e);
  process.exit(1);
});
