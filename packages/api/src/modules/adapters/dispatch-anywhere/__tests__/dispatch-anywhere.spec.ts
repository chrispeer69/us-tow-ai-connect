...
/**
 * Unit tests for the Dispatch Anywhere adapter. Playwright fully mocked — no
 * real browser, no network. Mirrors the Omadi spec layout.
 */
...
function makePage(cfg: {
  ...
  const url = cfg.landedUrl ?? 'https://app.dispatchanywhere.com/dispatch';
...
describe('DispatchAnywhereAdapter', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('login caches storage state in Redis under session:dispatchanywhere:<tenantId>', async () => {
    h.page = makePage({ landedUrl: 'https://app.dispatchanywhere.com/dispatch' });
    const redis = makeRedis(null);
    const adapter = new DispatchAnywhereAdapter(redis);

    await adapter.login('tenant-1', { username: 'u', password: 'p' });

    expect(redis.set).toHaveBeenCalledWith(
      'session:dispatchanywhere:tenant-1',
  ...
  it('scrapeAllActiveJobs throws SessionExpiredException when no session exists', async () => {
    h.page = makePage({});
    const adapter = new DispatchAnywhereAdapter(makeRedis(null));
  ...
  it('scrapeAllActiveJobs returns rows from the evaluated DOM', async () => {
    ...
    const adapter = new DispatchAnywhereAdapter(makeRedis(SESSION));

    const jobs = await adapter.scrapeAllActiveJobs('tenant-1');
    expect(jobs).toHaveLength(1);
    expect(jobs[0].jobId).toBe('DA-1');
  });

  it('scrapeAllActiveJobs deletes session + throws when page bounces to /login', async () => {
    h.page = makePage({ landedUrl: 'https://app.dispatchanywhere.com/login' });
    ...
    expect(redis.del).toHaveBeenCalledWith('session:dispatchanywhere:tenant-1');
  });
  ...
  it('acceptJob clicks the Accept button and returns a success result', async () => {
    ...
    const result = await adapter.acceptJob('tenant-1', 'DA-1');
  ...
  it('declineJob clicks the Decline button and returns a success result', async () => {
    ...
    const result = await adapter.declineJob('tenant-1', 'DA-1', 'too far');
    expect(result.success).toBe(true);
    expect(h.page.getByRole).toHaveBeenCalledWith(
      'button',
  ...
  it('dispatchJob returns a structured not-applicable result (never throws)', async () => {
    ...
    const result = await adapter.dispatchJob('tenant-1', {
      customer: 'Jane',
      address: '1 Main',
    });
    expect(result.success).toBe(false);
