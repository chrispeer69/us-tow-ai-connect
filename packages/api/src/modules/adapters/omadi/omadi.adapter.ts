...
// Best-effort row selectors — Omadi DOM has not been verified against a live
// account (no credentials at adapter-build time, see docs/ADAPTER_SELECTORS.md).
// First selector that resolves > 0 rows wins.
...
@Injectable()
export class OmadiAdapter implements TowingSoftwareAdapter {
  private readonly logger = new Logger(OmadiAdapter.name);
  private readonly LOGIN_URL = 'https://app.omadi.com/login';
  private readonly DISPATCH_URL = 'https://app.omadi.com/dispatch';
  ...
  async login(tenantId: string, creds: DecryptedCredentials): Promise<void> {
    ...
    try {
      ...
      await this.redis.set(
        `session:omadi:${tenantId}`,
        JSON.stringify(storageState),
        'EX',
        SESSION_TTL_SECONDS,
      );

      this.logger.log(`Omadi login successful for tenant ${tenantId}`);
    } catch (error) {
      this.logger.error(`Omadi login failed for tenant ${tenantId}: ${(error as Error).message}`);
      throw error;
    } finally {
  ...
  async scrapeAllActiveJobs(tenantId: string): Promise<ActiveJob[]> {
    const stateJson = await this.redis.get(`session:omadi:${tenantId}`);
    if (!stateJson) {
    ...
    try {
      ...
      await page.goto(this.DISPATCH_URL, { waitUntil: 'domcontentloaded', timeout: NAV_TIMEOUT_MS });

      if (page.url().includes('/login')) {
        await this.redis.del(`session:omadi:${tenantId}`);
      ...
      this.logger.log(`Omadi: scraped ${jobs.length} active jobs for tenant ${tenantId}`);
      return jobs;
    } finally {
      await browser.close().catch(() => undefined);
    }
  ...
  async testConnection(creds: DecryptedCredentials): Promise<AdapterConnectionTestResult> {
    ...
    try {
      ...
      return {
        success: true,
        message: 'Omadi connected successfully',
        latencyMs: Date.now() - start,
      };
    } catch (error) {
      return {
        success: false,
        message: `Omadi login failed: ${(error as Error).message}`,
        latencyMs: Date.now() - start,
      };
    } finally {
      await browser.close().catch(() => undefined);
  ...
  /**
   * Push a job from us → Omadi. Omadi is primarily a dispatch-software product
   * (operators manage their own jobs inside it), not a motor-club intake
   * broker with a verified public write API. Returns a structured
   * not-applicable result — same pattern as Towbook/TowLogs.
   */
  async dispatchJob(
    tenantId: string,
    ...
    this.logger.log(
      `[omadi] dispatchJob not-applicable (tenant=${tenantId}, payload-keys=${Object.keys(jobPayload).join(',')})`,
    );
    return {
      success: false,
      error: 'not-applicable: Omadi has no verified outbound dispatch write surface',
    };
  }
  ...
  private async performAction(
    ...
    const stateJson = await this.redis.get(`session:omadi:${tenantId}`);
    if (!stateJson) {
      const error = `credentials-not-configured: no Omadi session for tenant ${tenantId} — login required before ${kind}`;
      this.logger.warn(`[omadi] ${kind}Job: ${error}`);
    ...
    try {
      ...
      if ((await jobLink.count()) === 0) {
        ...
        return {
          success: false,
          error: `job ${sourceJobId} not found in Omadi dispatch list`,
        };
      }
      ...
      if (!primary) {
        ...
        return {
          success: false,
          error: `${kind} button not found on Omadi job ${sourceJobId} (tried: ${buttonNames.join(', ')}) — selectors may need verification, see docs/ADAPTER_SELECTORS.md`,
        };
      }
      ...
      this.logger.log(
        `[omadi] ${kind} succeeded for job ${sourceJobId} (tenant ${tenantId}): ${evidence}`,
      );
      return {
        success: true,
      ...
      this.logger.error(`[omadi] ${kind}Job failed for ${sourceJobId}: ${error}`);
  ...
  private async screenshotFailure(page: Page, kind: string, sourceJobId: string): Promise<void> {
    ...
    const file = path.join(os.tmpdir(), `omadi-${kind}-${safe}-${Date.now()}.png`);
    await page.screenshot({ path: file, fullPage: true }).catch(() => undefined);
    this.logger.warn(`[omadi] ${kind} failure screenshot: ${file}`);
  }
