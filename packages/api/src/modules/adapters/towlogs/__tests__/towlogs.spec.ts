...
/**
 * Unit tests for the TowLogs adapter. Playwright is fully mocked — no real
 * browser, no network. Covers:
 *  - login persists session to Redis
 *  - listOpenJobs returns parsed rows
 *  - accept/decline success + failure return shapes
 *  - credentials-not-configured path (no session)
 *  - screenshot on failure
 *  - dispatchJob structured not-applicable result
 ...
 * resolves visible+enabled. `jobLinkCount` controls whether the row link is
 * found. `landedUrl` controls the post-nav url (used by session-bounce tests).
 */
function makePage(cfg: {
  ...
  const url = cfg.landedUrl ?? 'https://app.towlogs.com/jobs';
  ...
  // Default locator(...) resolves to "absent" — the adapter checks count() to
  // decide whether a reason field / dialog is present, so 0 means "skip the
  // optional flow" rather than "click into something that doesn't exist".
...
describe('TowLogsAdapter', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('login caches storage state in Redis under session:towlogs:<tenantId>', async () => {
    h.page = makePage({ landedUrl: 'https://app.towlogs.com/jobs' });
    const redis = makeRedis(null);
    const adapter = new TowLogsAdapter(redis);

    await adapter.login('tenant-1', { username: 'u', password: 'p' });

    expect(redis.set).toHaveBeenCalledWith(
      'session:towlogs:tenant-1',
  ...
  it('listOpenJobs throws SessionExpiredException when no session exists', async () => {
    h.page = makePage({});
    const adapter = new TowLogsAdapter(makeRedis(null));
    await expect(adapter.listOpenJobs('tenant-1')).rejects.toBeInstanceOf(SessionExpiredException);
  });

  it('listOpenJobs returns rows from the evaluated DOM', async () => {
    const fakeJobs = [
      {
        jobId: 'J-1',
        customerName: 'Jane',
    ...
    const adapter = new TowLogsAdapter(makeRedis(SESSION));

    const jobs = await adapter.listOpenJobs('tenant-1');
    expect(jobs).toHaveLength(1);
    expect(jobs[0].jobId).toBe('J-1');
  });

  it('listOpenJobs deletes session + throws when page bounces to /login', async () => {
    h.page = makePage({ landedUrl: 'https://app.towlogs.com/login' });
    const redis = makeRedis(SESSION);
    const adapter = new TowLogsAdapter(redis);

    await expect(adapter.listOpenJobs('tenant-1')).rejects.toBeInstanceOf(SessionExpiredException);
    expect(redis.del).toHaveBeenCalledWith('session:towlogs:tenant-1');
  });
  ...
  it('acceptJob clicks the Accept button and returns a success result', async () => {
    ...
    const result = await adapter.acceptJob('tenant-1', 'J-1');
  ...
  it('declineJob clicks the Decline button and returns a success result', async () => {
    ...
    const result = await adapter.declineJob('tenant-1', 'J-1', 'too far');
    expect(result.success).toBe(true);
    expect(result.confirmedAt).toBeTruthy();
    expect(h.page.getByRole).toHaveBeenCalledWith(
      'button',
