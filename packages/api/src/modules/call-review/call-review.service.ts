...
export class CallReviewService {
  ...
  private systemPrompt(): string {
    return [
      ...
      '  - Calls with scenario=unknown and eligible=false can still carry a CONDITIONAL offer:',
      '    when the destination could not be resolved before dialling, the script lets the',
      '    agent make the offer ONLY after the customer confirms on the call that the tow is',
      '    going to a repair shop. eligible=false is decided before the call from the ticket;',
      '    an offer appearing on such a transcript is the conditional offer working as',
      '    designed, not a funnel mismatch or a data-quality bug. Do not report it as either.',
